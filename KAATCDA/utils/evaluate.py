...
def test2(model, graph, graph_UIS, user_dict, n_params, args):
    ...
    with torch.no_grad():
        ...
        #计算所有得分并保存
        scores = model.rating(user_gcn_emb, entity_gcn_emb).detach().cpu()
        np.savetxt("result1.txt", scores, delimiter = ",")
        ...
        for u_batch_id in range(n_user_batchs):
            ...
            if batch_test_flag:
                ...
                i_g_embddings = entity_gcn_emb[item_batch]
                rate_batch = model.rating(u_g_embeddings, i_g_embddings).detach().cpu()#计算u*i得分
            # print("########################")
            # print("########################")
            # print("########################")
            # print("########################")
            # np.savetxt("result.txt", rate_batch)
            # print("########################")
            # print("########################")
            # print("########################")
            # print("########################")
